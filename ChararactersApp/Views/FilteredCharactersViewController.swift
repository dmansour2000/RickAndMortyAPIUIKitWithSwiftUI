...
//  ChararactersApp
//
//  Created by Dina Mansour  on 21/10/2024.
//
...
class FilteredCharactersViewController: UIViewController, UINavigationBarDelegate{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Task.detached { [self] in
            ...
            if await self.isNetworkConnected() {
                ...
                if await self.savedState == "Alive" {
                    await self.characters = characterService.filteredCharacters(filter: .alive)
                }else if await self.savedState == "Dead" {
                    await self.characters = characterService.filteredCharacters(filter: .dead)
                }else if await self.savedState == "Unknown" {
                    await self.characters = characterService.filteredCharacters(filter: .unknown)
                }
            }else{
...
extension FilteredCharactersViewController: UICollectionViewDelegate, UICollectionViewDataSource, PaginationDelegate {
